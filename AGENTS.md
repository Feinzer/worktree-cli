...
- **`shared add <path>`** moves a file/folder from a worktree into `.shared/` (sibling of the bare `.git`) and symlinks it back; `switch` re-creates the symlinks in every worktree it enters. **`shared list`** (alias `ls`) prints the manifest. **`shared remove <relpath>`** (alias `rm`) **destructively** deletes the `.shared/` copy and removes symlinks across all worktrees (real files are left untouched). The manifest is `.shared/.manifest` (one repo-relative path per line); it is deleted when the last entry is removed. Symlinks are **relative** (`../.shared/<relpath>`), so the repo dir stays portable. On `switch`, a real file/folder at a shared path is **skipped with a warning**, never clobbered.
...
- `shared add` resolves the path by matching the longest worktree-name prefix first (so `feature/shared-files` wins over `feature`), then falls back to the current worktree's toplevel. Absolute paths are rejected. The source must not already be a symlink, and the relpath must not already be in `.shared/` or the manifest.
- `shared remove` validates the relpath against the manifest, `rm -rf`s the `.shared/<relpath>` copy, rewrites the manifest (deleting it if empty), then walks every worktree removing symlinks at that relpath. Real files/folders are never clobbered. The deletion is destructive — no move-back.
- `switch` calls `_worktree_link_common` / `Invoke-WorktreeLinkCommon` after `cd`/`Set-Location`; it is idempotent (existing symlinks are left alone) and silent when no `.shared/.manifest` exists.
