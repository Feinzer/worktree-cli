...
- **`shared add <path>`** moves a file/folder from a worktree into `.common/` (sibling of the bare `.git`) and symlinks it back; `switch` re-creates the symlinks in every worktree it enters. **`shared list`** (alias `ls`) prints the manifest. **`shared remove <relpath>`** (alias `rm`) **destructively** deletes the `.common/` copy and removes symlinks across all worktrees (real files are left untouched). The manifest is `.common/.wt-shared` (one repo-relative path per line); it is deleted when the last entry is removed. Symlinks are **relative** (`../.common/<relpath>`), so the repo dir stays portable. On `switch`, a real file/folder at a shared path is **skipped with a warning**, never clobbered.
...
- `shared remove` validates the relpath against the manifest, `rm -rf`s the `.common/<relpath>` copy, rewrites the manifest (deleting it if empty), then walks every worktree removing symlinks at that relpath. Real files/folders are never clobbered. The deletion is destructive — no move-back.
